...
// ─── Auto-mark HQ todos done when agent run completes ───
async function markAgentTodoDone(agent, runId) {
  try {
    // Find pending todo matching this run_id
    if (runId) {
      const { data: matched } = await supabase.from('ops_todos')
        .select('id, title')
        .eq('run_id', runId)
        .eq('done', false)
        .limit(1);
      ...
      if (matched && matched.length > 0) {
        ...
        await supabase.from('ops_events').insert({
          agent,
          event_type: 'task',
          title: `✅ Todo auto-completed: ${matched[0].title.slice(0, 60)}`,
        });

        console.log(`[TodoDone] Auto-marked todo #${matched[0].id} done (runId: ${runId})`);
        return;
      }
    }

    // Fallback: match oldest pending todo assigned to this agent
    const { data: agentTodos } = await supabase.from('ops_todos')
      .select('id, title')
      .eq('agent', agent)
      .eq('done', false)
    ...
    if (agentTodos && agentTodos.length > 0) {
      await supabase.from('ops_todos').update({
        ...
        updated_at: new Date().toISOString(),
      }).eq('id', agentTodos[0].id);

      await supabase.from('ops_events').insert({
        agent,
        event_type: 'task',
        title: `✅ Todo auto-completed: ${agentTodos[0].title.slice(0, 60)}`,
      });

      console.log(`[TodoDone] Auto-marked todo #${agentTodos[0].id} done (agent: ${agent})`);
    }
  } catch (err) {
    console.error('[TodoDone] Error:', err.message);
  }
}
...
async function processGatewayMessage(msg) {
  try {
    ...
    if (eventName === 'agent' && payload.stream === 'lifecycle' && payload.data?.phase === 'end') {
      ...
      // Auto-mark HQ todo as done when agent run completes
